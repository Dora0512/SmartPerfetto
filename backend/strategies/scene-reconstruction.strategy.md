...
The optional reference `value`, when supplied, must preserve the exact captured JSON type and value: a numeric cell is not its string representation. An evidence boundary column must exist in the cited original row. If an end is computed as `ts + dur`, query and cite a real end column or mark the boundary inferred; never invent a column name. Use rejection diagnostics' segment ID, reference index and requested columns to repair the specific candidate before accumulating a larger batch.
