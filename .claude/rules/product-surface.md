...
| Critical-path wait chain | AI Assistant `Critical path` button on a selected `thread_state` -> `POST /api/critical-path/:traceId/analyze` | Web UI users inspecting one blocked task | Non-workspace route: 410 under enterprise/OIDC; trace ownership checked like Agent routes. The optional model summary is AI feature `critical_path_ai_summary`: AI off, a non-Claude active runtime, or missing credentials return the deterministic summary with a warning (never 403). It follows the caller's Provider Manager profile, runs the isolated one-shot SDK configuration, and aborts on client disconnect. The result is not persisted to reports, snapshots, or the conversation |
