...
聊天表格通过 SSE 发送有界预览：每张表最多 200 行，行与嵌套详情共用
256 KiB 的保守 JSON 数据预算。超出预算时只保留完整行的连续前缀，不改写单元格；
大详情可省略。界面显示预览行数与原始总行数，复制、收藏、排序仅作用于预览，
不提供前缀的全表统计。缺少预览行的本地引用检查保持“未核验”，后端完整证据核验
不受影响。实时事件和历史重放使用同一展示投影；原始 Skill 输出、报告与快照输入
仍保留完整数据。此预算不是整个事件或进程的内存上限，元数据与其他事件类型不在其中。
